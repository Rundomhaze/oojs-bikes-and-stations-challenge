...
- Станция должна позволять взять велосипеды в прокат (выдать один велосипед) ( метод release() )
  (метода release() должлжен возвращать undefined, если на станции не осталось велосипедов; должен выдавать велосипеды по принципу очереди). Из станции должны удаляться выданные велосипеды.
